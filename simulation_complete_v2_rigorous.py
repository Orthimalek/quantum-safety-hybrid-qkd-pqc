...
=============================================================================
Authors : Vinit Datta Rane  (vinitrane@ieee.org)  [first author]
          Shuchona Malek Orthi  (shuchona@ieee.org)  [second author]
          Mehrdad S. Sharbaf  (msharbaf@sharbafandassociates.com)  [third author]
          ORCID: 0009-0004-9740-8009 ; 0009-0007-5397-4561
...
QBER_THRESH    = 0.025
EPS_SEC        = 1e-10
# Composable epsilon-budget (union bound: eps_sec <= eps_PE+eps_EC+eps_PA+eps_smooth)
EPS_PE         = EPS_SEC/4   # parameter estimation
EPS_EC         = EPS_SEC/4   # error correction / reconciliation
EPS_PA         = EPS_SEC/4   # privacy amplification
EPS_BAR        = EPS_SEC/4   # smooth min-entropy parameter
MLDSA_SIGN_MS  = 0.23
MLDSA_VER_MS   = 0.19
N_EXCHANGES    = 4
ROUND_S        = 1.0
...
def finite_key_length(n_raw, e_bit):
    """Heuristic sqrt(n) bound — retained for comparison only (conservative)."""
...
def finite_key_length_rigorous(n_raw, e_bit):
    """
    Rigorous composable finite-key bound (Tomamichel et al., Nat. Commun. 3:634, 2012;
    Renner 2008; Hayashi-Tsurumaru 2012). Secret key length:

        l = n_key*[1 - h(e_phase + mu)] - leak_EC - Delta_PA - Delta_PE

    where mu is the statistical-fluctuation deviation on the phase error from
    parameter estimation on the verification subset, leak_EC is the leaked
    reconciliation information, and Delta_PA, Delta_PE are the
    privacy-amplification and smoothing penalties from the epsilon-budget.
    """
    n_key = n_raw * (1-VERIF_FRAC)
    n_pe  = n_raw * VERIF_FRAC          # verification (parameter-estimation) sample
    if n_key <= 0 or n_pe <= 0:
        return 0.0
    # Statistical fluctuation on phase error (Serfling/Hoeffding-type bound on PE sample)
    mu = np.sqrt(np.log(1.0/EPS_PE) / (2.0*n_pe))
    e_phase = min(e_bit + mu, 0.5)
    # Leaked info during error correction (LDPC inefficiency f)
    leak_EC = EC_EFF * h(e_bit) * n_key + np.log2(2.0/EPS_EC)
    # Privacy amplification + smoothing penalty (leftover hash lemma)
    delta_PA = 2.0*np.log2(1.0/(2.0*EPS_PA))
    delta_smooth = 7.0*np.sqrt(n_key*np.log2(2.0/EPS_BAR))  # smooth-entropy correction
    # Devetak-Winter / Tomamichel composable key length
    ell = n_key*(1.0 - h(e_phase)) - leak_EC - delta_PA - delta_smooth
    return max(0.0, ell)
...
for d in [10,30,50,75,100]:
    ...
    n_min=next((n for n in n_arr if finite_key_length_rigorous(n,e_bit)>0),None)
    n_min_heur=next((n for n in n_arr if finite_key_length(n,e_bit)>0),None)
...
print(f"  [rigorous n_min={n_min:.3e} bits;  heuristic n_min={n_min_heur:.3e} bits "
      f"(comparison only)]")
